...
namespace DataBaseFirst.Repository
{
    public class LibroRepository : ILibro
    {
        ...
        public async Task<Libro> BuscarPorTitulo(string titulo)
        {
            ...
            return await Task.Run(() => _dbContext.Libros.FromSqlRaw("EXEC PA_BUSCAR_TITULO_LIBRO @Titulo", id).AsNoTracking().AsEnumerable().FirstOrDefault());
        }
        ...
        {
            return await _dbContext.Database.ExecuteSqlRawAsync("EXEC PA_REGISTRAR_LIBRO @Titulo, @Id_Genero, @Id_Autor, @Numero_Paginas, @Fecha_Publicacion, @Estado",
                new SqlParameter("@Titulo", libro.Titulo ?? (object)DBNull.Value),
                new SqlParameter("@Id_Genero", libro.IdGenero ?? (object)DBNull.Value),
        ...
        {
            return await _dbContext.Database.ExecuteSqlRawAsync("EXEC PA_EDITAR_LIBRO @Id_Libro, @Titulo, @Id_Genero, @Id_Autor, @Numero_Paginas, @Fecha_Publicacion, @Estado",
