...
namespace DataBaseFirst.Repository
{
    public class PrestamoRepository : IPrestamo
    {
        ...
        public async Task<Prestamo> ObtenerPorUsuario(int idUsuario)
        {
            ...
            return await Task.Run(() => _dbContext.Prestamos.FromSqlRaw("EXEC PA_BUSCAR_PRESTAMO_USUARIO @Id_Usuario", idUsuario).AsNoTracking().AsEnumerable().FirstOrDefault());
        }

        public async Task<Prestamo> ObtenerPorLibro(int idLibro)
        {
            var id = new SqlParameter("@Id_Libro", idLibro);
            return await Task.Run(() => _dbContext.Prestamos.FromSqlRaw("EXEC PA_BUSCAR_PRESTAMO_LIBRO @Id_Libro", idLibro).AsNoTracking().AsEnumerable().FirstOrDefault());
        }
